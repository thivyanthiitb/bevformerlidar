...
class BEVFormerV2(MVXTwoStageDetector):
    ...
    def extract_pts_feat(self, pts):
        ...
        if self.with_pts_neck:
            ...
            # BUG: cloning due to relu-inplace-error somewhere in fpn. 
            # TODO: this needs a better fix
            x_clone = [xi.clone() for xi in x]
            x = self.pts_neck(x_clone) 
            
        return x

    def extract_feat(self, points, img, img_metas, len_queue=None):
        """Extract features from images and points."""

        img_feats = self.extract_img_feat(img)
        pts_feats = self.extract_pts_feat(points) if self.use_lidar else None

        # TODO: take argument as only the condition 
        # below to be true or not. 
        # Right now, useless copy is being done. 
    ...
    def obtain_history_bev(self, pts_dict, img_dict, img_metas_dict):
        ...
        with torch.no_grad():
            for t in img_dict.keys():
                ...
                img_feats, pts_feats = self.extract_feat(points=pts, img=img, img_metas=img_metas)
                if self.num_levels:
                    img_feats = img_feats[:self.num_levels]
                bev = self.pts_bbox_head(
                    pts_feats, img_feats, img_metas, None, only_bev=True)
    ...
    def forward_train(self,
        ...
        for ind, t in enumerate(img_metas.keys()):
            ...
            for b in range(len(points)):
                pts_batches.append(points[b][ind])
            pts_dict[t] = pts_batches
        ...
        prev_bev = self.obtain_history_bev(pts_dict, img_dict, prev_img_metas)
        ...
        losses_pts = self.forward_pts_train(
            pts_feats, 
            img_feats if self.num_levels is None
        ...
        if self.fcos3d_bbox_head:
            losses_mono = self.forward_mono_train(
                img_feats=img_feats if self.num_mono_levels is None
    ...
    def forward_test(self, img_metas, img=None, pts=None, **kwargs):
        ...
        new_prev_bev, bbox_results = self.simple_test(img_metas[0], img[0], pts[0], prev_bev=None, **kwargs)
        return bbox_results

    def simple_test_pts(self, pts_feats, img_feats, img_metas, prev_bev=None, rescale=False):
        """Test function"""
        outs = self.pts_bbox_head(pts_feats, img_feats, img_metas, prev_bev=prev_bev)
    ...
    def simple_test(self, img_metas, img=None, pts=None, prev_bev=None, rescale=False, **kwargs):
        ...
        img_metas = [img_metas[0], ]
        img_feats = self.extract_feat(points=pts, img=img, img_metas=img_metas)
        if self.num_levels:
            img_feats = img_feats[:self.num_levels]
